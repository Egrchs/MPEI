...
async def test_default (dut):
    ...
    outputs_model['gnt1']         = 0b0
    outputs_model['gnt2']         = 0b1
    outputs_model['sb_masters']   = 0b10
    outputs_model['sb_mastlock']  = 0b0
    ...
    dut.req2.value = 1
    await ClockCycles(dut.clk , 2)
    outputs_model['RDATA']        = 0b00000000000000000000000000001011
    outputs_model['HADDR']        = 0b00010011110011
    outputs_model['sel_0']        = 0b1
    outputs_model['sel_1']        = 0b0
    outputs_model['sel_slave']    = 0b0
    readOutputs (outputs_result, dut)
    for key in outputs_model:
        AssertionString = f"Ожидали {outputs_model[key]} а получили {outputs_result[key]} "
        assert outputs_result[key] == outputs_model[key] , AssertionString
    ...
    dut.req2.value = 1
    await ClockCycles(dut.clk , 2)
    outputs_model['RDATA']        = 0b00000000000001010001111111000011
    outputs_model['HADDR']        = 0b10010011110011
    outputs_model['sel_0']        = 0b0
    outputs_model['sel_2']        = 0b1
    outputs_model['sel_slave']    = 0b10
    readOutputs (outputs_result, dut)
    for key in outputs_model:
        AssertionString = f"Ожидали {outputs_model[key]} а получили {outputs_result[key]} "
        assert outputs_result[key] == outputs_model[key] , AssertionString    
